...
public interface FinanceDao extends JpaRepository<Finance, Integer> {
	...
	@Transactional
	@Modifying
	@Query(value = "insert into finance (title, detail, price, build_date) "
			+ "select :title, :detail, :price, :buildDate "
			+ "where not exists (select 1 from finance "
			+ "where title = :title and detail = :detail and build_date = :buildDate)", nativeQuery = true)
	...
	@Query(value = "select * from finance "
			+ "where title = :title and month(build_date) = :month", nativeQuery = true)
	 ...
	 */
	@Query(value = "select * from finance "
			+ "where detail = :detail and month(build_date) = :month", nativeQuery = true)
	 ...
	 */
	@Query(value = "select * from finance "
			+ "where month(build_date) = :month", nativeQuery = true)
